...
/*
void set_PEC_conditions_2d(int *ia, int *ja, doublecomplex *a, int nnz, int *boundary_info, bool xperiodic, bool yperiodic, int nedges)
{
    ...
    // int *boundary_info: an array of length nedges containing the index of the boundary on which
    //      each edge lies, or -1 if the edge does not lie on any boundary.
    // bool xperiodic, bool yperiodic: boolean values expressing the periodicity of the problem.
    ...
    for(i = 0; i < nnz; i++)
    {
        ...
        if(!xperiodic)
        {
            ...
            if((boundary_info[row] == 0 || boundary_info[row] == 2) ||
                (boundary_info[col] == 0 || boundary_info[col] == 2))
                {
                    a[i].re = 0;
                    a[i].i = 0;
                }
        ...
        {
            // Same as above.
            if((boundary_info[row] == 1 || boundary_info[row] == 3) ||
                (boundary_info[col] == 1 || boundary_info[col] == 3))
                {
                    a[i].re = 0;
                    a[i].i = 0;
                }
...
void set_periodic_conditions(int *ia, int *ja, doublecomplex *a, int nnz, int nedges, int nbound, int *source, int *destination, byte *orientations, doublecomplex phase)
{
...
int purge_matrix(int **im, int **jm, doublecomplex **m, int *nnza, int *nKeep, int **keep_indices )
{
    ...
    // while keep_indices will be an array of size nKeep containing the indices retained.
    // INPUT:
    // int **im, int **jm, doublecomplex **m: pointers to the
    //      arrays containing matrix information.
    // int *nnza: pointer to an int containing the size of the arrays.
    ...
    // int **keep_indices: pointer to an array containing the indices of the non-empty rows and columns
    //      found.
    ...
    *keep_indices = realloc(keep, *nKeep*sizeof(int));
