...
int read_grid_file_3d(char *filename, point3d **grid, int *gridn)
{
    ...
    *grid = malloc(*gridn*sizeof(point3d));
    for(i = 0; i < *gridn; i++)
    {
        ...
        if(!scan_line(fid, line))
            return 0;
        sscanf(line, "%lg %lg %lg", &((*grid)[i].x), &((*grid)[i].y), &((*grid)[i].z));
    }
...
// The following 2 functions are used to write the results (eigenfrequencies and
// e-field) to file. They are quite self-explanatory.
...
int write_modes_3d(char *filename, field3d *modes, point3d *grid, int gridn, int nev, int kvals)
{
    FILE *fid;
    int k, i,j;
    ...
    for(k = 0; k < kvals; k++)
        for(i = 0; i < nev; i++)
        {
            for(j = 0; j < gridn; j++)
            {
                fprintf(fid, "%+g%+gi ", modes[(k*nev + i)*gridn + j].z.re, modes[(k*nev + i)*gridn + j].z.i);
            }
            fprintf(fid, "\r\n");
        }
...
int read_mesh_file_3d(char* filename, point3d** points, int* npoints, edge** edges, int* nedges, tetra** tetras, int* ntetras)
{
    // Reads a COMSOL mesh file containing a 3-dimensional mesh composed by tetrahedra.
    // INPUT:
    ...
    // point3d** points: a pointer to an array containing the information on the nodes.
    ...
    // tetra** tetras: a pointer to an array containing the information on the tetrahedra (4-node elements).
    // int* ntetras: a pointer to an integer containing the number of tetrahedra.
    // ERROR CODES:
    ...
    *npoints = atoi(line);
    ...
    pnts = malloc(*npoints * sizeof(point3d));
    for (i = 0; i < *npoints; i++)
    {
        scan_line(fid, line);
        sscanf(line, "%lf %lf %lf", &pnts[i].x, &pnts[i].y, &pnts[i].z);
    ...
    for (i = 0; i < num_el_types; i++)
    {
        ...
        {
            // Tetrahedron
            tets = malloc(num_el*sizeof(tetra));
            current = (void*)tets;
            *ntetras = num_el;
            size_el = sizeof(tetra);
